...
Install_SSR(){
	...
	Installation_dependency
	echo -e "${Info} 开始/安装 ShadowsocksR文件..."
	Download_SSR
	echo -e "${Info} 开始安装 ShadowsocksR服务脚本(init)..."
	Service_SSR
...
latestport(){
        ...
        do
                ...
                Get_User_transfer "${user_port}"
                transfer_enable_Used_233=$(expr $transfer_enable_Used_233 + $transfer_enable_Used_2_1)
                user_list_all="${user_port}"
        done
        Get_User_transfer_all
       # echo && echo -e "=== 用户总数 ${Green_background_prefix} "${user_total}" ${Font_color_suffix}"
        #echo -e ${user_list_all}
	latestport=${user_list_all:0-4}
	newport=`expr $latestport + 1 `
	#echo ${user_port}
    echo $newport
}
...
adduser(){
    ...
    portport=$3
    #echo $portport                                              
#python mujson_mgr.py -a -u "${ssr_user}" -p "${newport}" -k "${ssr_password}" -m aes-256-cfb -O origin -G 2 -o plain -s 2048 -S 4096 -t 1000 -f 3306 | grep -w "add user info"
python mujson_mgr.py -a -u "${ssr_user}" -p "${portport}" -k "${ssr_password}" -m aes-256-cfb -O origin -G 2 -o plain -s 2048 -S 4096 -t 1000 -f 3306 | grep -w "add user info"
}
...
Add_iptab(){
		...
		iptables -I INPUT -m state --state NEW -m tcp -p tcp -m multiport --dports 2333:2600 -j ACCEPT
		iptables -I INPUT -m state --state NEW -m udp -p udp -m multiport --dports 2333:2600 -j ACCEPT
		ip6tables -I INPUT -m state --state NEW -m tcp -p tcp -m multiport --dports 2333:2600 -j ACCEPT
		ip6tables -I INPUT -m state --state NEW -m udp -p udp -m multiport --dports 2333:2600 -j ACCEPT
...
if [[ "${action}" == "userinfo" ]]; then
	...
	cd /usr/local/shadowsocksr/
	adduser $2 $3 $4
elif [[ "${action}" == "usershow" ]]; then
        cd /usr/local/shadowsocksr
        List_port_user
elif [[ "${action}" == "actir" ]]; then
      #  cd /usr/local/shadowsocksr
       active $2 
elif [[ "${action}" == "activeuser" ]]; then
       cd /usr/local/shadowsocksr
       set_transfer $2 $3
elif [[ "${action}" == "iptable" ]]; then
      #  cd /usr/local/shadowsocksr
   	Add_iptab
	Save_iptables 
elif [[ "${action}" == "hiptable" ]]; then
     #  cd /usr/local/shadowsocksr
   	deliptable $2 
elif [[ "${action}" == "latestport" ]]; then
		cd /usr/local/shadowsocksr
       latestport                                              
